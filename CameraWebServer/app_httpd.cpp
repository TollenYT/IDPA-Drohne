...
// Add this near where stream_httpd is initialized
static esp_err_t joystick_handler(httpd_req_t *req) {
  ...
  if (buf_len > 1) {
    if (httpd_req_get_url_query_str(req, buf, buf_len) == ESP_OK) {
      char param_x[10], param_y[10], param_r[10], param_s[10];
      int x = 0, y = 0, r = 0, s = 255;  // Default values
      ...
      if (httpd_query_key_value(buf, "r", param_r, sizeof(param_r)) == ESP_OK) {
        r = atoi(param_r);
      }

      // Parse Speed parameter,
      if (httpd_query_key_value(buf, "s", param_s, sizeof(param_s)) == ESP_OK) {
        s = atoi(param_s);
      }
      ...
      if (Serial) {  // Check if Serial is available
        ...
        char command[50];
        snprintf(command, sizeof(command), "%d:%d:%d:%d", x, y, r, s);
        Serial.println(command);

        log_i("Joystick command: x=%d, y=%d, r=%d, s=%d", x, y, r, s);
      } else {
        log_e("Serial not available");
      }
    }
  }

  // Enable CORS for external web interface
  httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
  httpd_resp_send(req, "OK", HTTPD_RESP_USE_STRLEN);
  return ESP_OK;
}

// Handler for rotation button presses
static esp_err_t rotation_button_handler(httpd_req_t *req) {
  char buf[100];
  size_t buf_len = httpd_req_get_url_query_len(req) + 1;

  if (buf_len > 1) {
    if (httpd_req_get_url_query_str(req, buf, buf_len) == ESP_OK) {
      char param_r[10], param_s[10];
      int r = 0, s = 255;  // Default values
      ...
      if (Serial) {
        // Send only rotation command to Arduino
        char command[30];
        snprintf(command, sizeof(command), "ROT:%d:%d", r, s);
        Serial.println(command);

        log_i("Rotation command: r=%d, s=%d", r, s);
      } else {
        log_e("Serial not available");
      }
    }
...
void startCameraServer() {
  ...
  httpd_uri_t joystick_uri = {
    ...
    .supported_subprotocol = NULL
      ...
      httpd_uri_t rotation_button_uri = {
      .uri = "/rotation",
      .method = HTTP_GET,
      .handler = rotation_button_handler,
      .user_ctx = NULL
  #ifdef CONFIG_HTTPD_WS_SUPPORT
      ,
      .is_websocket = true,
  ...
  if (httpd_start(&camera_httpd, &config) == ESP_OK) {
    ...
    // Add this line to register the new joystick handler and rotation handler
    httpd_register_uri_handler(camera_httpd, &joystick_uri);
    httpd_register_uri_handler(camera_httpd, &rotation_button_uri);
